...
async function main() {
  ...
  const layoutFixPdfPath = path.join(genDir, `OrgFlow_A3_Organization_Chart_LAYOUTFIX_${timestamp}.pdf`);
  const verifyPdfPath = path.join(genDir, `OrgFlow_A3_Organization_Chart_VERIFY_20260823_1416.pdf`);
  const refPdfPath = path.resolve('org data/reference/Org.FY2026_Rev.2.pdf');

  const oldHash = getSha256(verifyPdfPath);

  console.log('Generating new timestamped LAYOUTFIX PDF from current domain/print engine...');
  const pdfBytes = await generateA3OrganizationChartPDF(
    ...
    dataset.employees,
    {
      ...
      documentId: `OFS-A3-LAYOUTFIX-${timestamp}`,
  ...
  fs.writeFileSync(layoutFixPdfPath, pdfBytes);
  const newHash = getSha256(layoutFixPdfPath);
  const stats = fs.statSync(layoutFixPdfPath);

  console.log(`[LAYOUTFIX PDF CREATED] ${layoutFixPdfPath}`);
  console.log(`Size: ${stats.size} bytes`);
  console.log(`Modified: ${stats.mtime.toISOString()}`);
  console.log(`Old SHA-256 (VERIFY):    ${oldHash}`);
  console.log(`New SHA-256 (LAYOUTFIX): ${newHash}`);

  // Generate comparison PNGs
  const refPngPath = path.join(compDir, `reference_LAYOUTFIX_${timestamp}.png`);
  const genPngPath = path.join(compDir, `generated_LAYOUTFIX_${timestamp}.png`);
  const sideBySidePath = path.join(compDir, `side_by_side_LAYOUTFIX_${timestamp}.png`);
  const overlayPath = path.join(compDir, `overlay_LAYOUTFIX_${timestamp}.png`);

  console.log('Rendering reference and layoutfix PNGs...');
  await renderPdfPageToPng(refPdfPath, refPngPath, 1.5);
  await renderPdfPageToPng(layoutFixPdfPath, genPngPath, 1.5);
